#!/bin/sh
kernel_version="$(apt info linux-headers-truenas-amd64 | awk '/Source:/ { print $2}' | sed 's/linux-//')"
kernel_package_version="$(apt info linux-headers-truenas-amd64 | awk '/Version:/ { print $2}' | sed 's/linux-//')"
...
do
    ...
    sed -i.bak "s/@KERNEL_PACKAGE_VERSION@/$kernel_package_version/g" $file
